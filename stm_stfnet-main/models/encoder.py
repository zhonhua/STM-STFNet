...
class Down(nn.Module):
    def __init__(self, down_scale=2, in_dim=64, depths=(2, 2, 6, 2)):
        ...
        self.inc = PatchEmbed(img_size=128, patch_size=down_scale, in_chans=18, embed_dim=in_dim,
                              norm_layer=nn.LayerNorm)
        self.down1 = DownBlock(in_channels=in_dim, out_channels=in_dim * 2, resolution=128 // down_scale,
                               downsample=PatchMerging, cur_depth=depths[0])
        self.down2 = DownBlock(in_channels=in_dim * 2, out_channels=in_dim * 4, resolution=64 // down_scale,
                               downsample=PatchMerging, cur_depth=depths[1])
        self.down3 = DownBlock(in_channels=in_dim * 4, out_channels=in_dim * 8, resolution=32 // down_scale,
                               downsample=PatchMerging, cur_depth=depths[2])
        self.down4 = DownBlock(in_channels=in_dim * 8, out_channels=in_dim * 8, resolution=16 // down_scale,
