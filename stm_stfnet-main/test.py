...
def test(opt, model, test_dates, IMAGE_SIZE, PATCH_SIZE):
    ...
    for i in range(len(h_index_list)):
        for j in range(len(w_index_list)):
            ...
            output,output1,output2= model(ref1_lr, ref1_hr,ref2_lr,ref2_hr,input_lr)
    ...
    for real_predict in [real_output]:
        ...
        print('[%s/%s/%s] RMSE: %.4f SSIM: %.4f UIQI: %.4f CC: %.4f ERGAS: %.4f SAM: %.4f' % (
            cur_data1, ref_data[0], ref_data[1], np.mean(np.array(cur_result['rmse'])),
    ...
    save_dir = 'E:\Github\STM-STFNet\\best'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    im_name = os.path.join(save_dir, 'swin_LGCbest.tif')
...
def train(opt, train_dates, test_dates, IMAGE_SIZE, PATCH_SIZE):
    ...
    model_CKPT = torch.load('E:\Github\TCswinstfm_LGC_Final\experiment_best\epoch_best.pth')
...
def main():
    ...
    parser.add_argument('--train_dir', default='D:\qzh\LGC_Train_128', help='Datasets train directory')
