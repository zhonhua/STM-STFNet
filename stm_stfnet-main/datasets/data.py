...
def get_pair_path(root_dir, target_dir_name, ref_dir_name):
    ...
    target_dir = root_dir + '/' + ref_dir_name[0]
    for filename in os.listdir(target_dir):
        if filename[:3] == 'MOD':
            paths[2] = os.path.join(target_dir, filename)
        else:
            paths[3] = os.path.join(target_dir, filename)
    target_dir = root_dir + '/' + ref_dir_name[1]
    for filename in os.listdir(target_dir):
        if filename[:3] == 'MOD':
            paths[4] = os.path.join(target_dir, filename)
        else:
            paths[5] = os.path.join(target_dir, filename)
    return paths
...
class PatchSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        gt_mask = masks[0] * masks[1] * masks[2] * masks[3]*masks[4]*masks[5]

        return patches[0], patches[1], patches[2], patches[3],patches[4],patches[5],gt_mask
    def __len__(self):
