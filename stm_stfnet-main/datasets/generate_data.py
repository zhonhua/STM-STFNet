...
def main():
    ...
    output_dir = 'D:\qzh\LGC_Train_128'
    ...
    for k in tqdm(range(len(train_dates))):
        ...
        for filename in os.listdir(target_dir):
            if filename[:3] != 'MOD':
                ...
                total_original_images[k, 1] = np.load(path,allow_pickle=True)
            else:
                path = os.path.join(target_dir, filename)
                total_original_images[k, 0] = np.load(path,allow_pickle=True)
    ...
    for k in tqdm(range(len(train_dates))):
        for i in range(len(h_index_list)):
            for j in range(len(w_index_list)):
                ...
                ref_index1 = k
                while ref_index1 == k:
                    ref_index1 = np.random.choice(len(train_dates))
                ref_index2=k
                while ref_index2==k:
                    ref_index2=np.random.choice(len(train_dates))
                images = []
                images.append(total_original_images[k, 0])
                images.append(total_original_images[k, 1])
                images.append(total_original_images[ref_index1, 0])
                images.append(total_original_images[ref_index1, 1])
                images.append(total_original_images[ref_index2, 0])
                images.append(total_original_images[ref_index2, 1])
