...
def test(opt, model, test_dates, IMAGE_SIZE, PATCH_SIZE):
    ...
    for cur_data in test_dates:
        ...
        if cur_day == 347 :
            cur_data1=cur_data
            for ref_data1 in test_dates:
                ref_day = int(ref_data1.split('_')[1])
                if ref_day != cur_day:
                    ref_data.append(ref_data1)

    images = load_image_pair(opt.root_dir, cur_data1, ref_data)
    ...
    for i in range(len(h_index_list)):
        for j in range(len(w_index_list)):
            ...
            ref1_lr = images[2][:, h_start: h_start + PATCH_SIZE, w_start: w_start + PATCH_SIZE]
            ref1_hr = images[3][:, h_start: h_start + PATCH_SIZE, w_start: w_start + PATCH_SIZE]
            ref2_lr = images[4][:, h_start: h_start + PATCH_SIZE, w_start: w_start + PATCH_SIZE]
            ref2_hr = images[5][:, h_start: h_start + PATCH_SIZE, w_start: w_start + PATCH_SIZE]

            flip_num = 0
            rotate_num0 = 0
            rotate_num = 0
            input_lr, im_mask = transform_image(input_lr, flip_num, rotate_num0, rotate_num)
            ref1_lr, im_mask = transform_image(ref1_lr, flip_num, rotate_num0, rotate_num)
            ref1_hr, im_mask = transform_image(ref1_hr, flip_num, rotate_num0, rotate_num)
            ref2_lr, im_mask = transform_image(ref2_lr, flip_num, rotate_num0, rotate_num)
            ref2_hr, im_mask = transform_image(ref2_hr, flip_num, rotate_num0, rotate_num)

            input_lr = input_lr.unsqueeze(0).cuda()
            ref1_lr = ref1_lr.unsqueeze(0).cuda()
            ref1_hr = ref1_hr.unsqueeze(0).cuda()
            ref2_lr = ref2_lr.unsqueeze(0).cuda()
            ref2_hr = ref2_hr.unsqueeze(0).cuda()

            output = model(ref1_lr, ref1_hr,ref2_lr,ref2_hr,input_lr)
            output=output[0]
            output = output.squeeze()

            # Determine the four coordinates of the fill image
    ...
    for real_predict in [real_output]:
        ...
        print('[%s/%s/%s] RMSE: %.4f SSIM: %.4f UIQI: %.4f CC: %.4f ERGAS: %.4f SAM: %.4f PSNR: %.4f' % (
            ref_data[0],cur_data1 ,ref_data[1], np.mean(np.array(cur_result['rmse'])),
...
def train(opt, train_dates, test_dates, IMAGE_SIZE, PATCH_SIZE):
    ...
    best_ssim = 0.0
    best_epoch = -1
    save_dir = 'E:\Github\STM-STFNet\experiment_best'
    ...
    for epoch in range(opt.num_epochs):
        ...
        for item, (target_lr, target, ref1_lr, ref1_target,ref2_lr, ref2_target,gt_mask) in enumerate(train_loader):
            t_start = timer()

            target_lr= target_lr.cuda()
            target = target.cuda()
            ref1_lr = ref1_lr.cuda()
            ref1_target = ref1_target.cuda()
            ref2_lr = ref2_lr.cuda()
            ref2_target = ref2_target.cuda()
            gt_mask = gt_mask.float().cuda()

            predict_fine,ff2,fb2= model(ref1_lr, ref1_target, ref2_lr, ref2_target,target_lr)


            optimizer.zero_grad()

            # pixel loss
            l_total = cri_pix(predict_fine*gt_mask , target*gt_mask , ff2*gt_mask, fb2*gt_mask,is_ds=False)
...
def main():
    ...
    parser.add_argument('--root_dir', default='E:\Github\LGC\LGC_npy', help='Datasets root directory')
    parser.add_argument('--train_dir', default='E:\Github\LGC\LGC_Train_128', help='Datasets train directory')
